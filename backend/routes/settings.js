...
router.post('/patterns', async (req, res) => {
  try {
    ...
    // Get current settings
    const currentResult = await db.get('SELECT settings_json FROM settings WHERE id = 1');
    let currentSettings = currentResult ? JSON.parse(currentResult.settings_json) : {
      pollingInterval: 60,
      patternCategories: {}
    };
    ...
    if (currentSettings.patternCategories.pressure && !currentSettings.patternCategories.pressure.notificationInterval) {
    ...
    await db.run(
      'INSERT OR REPLACE INTO settings (id, settings_json) VALUES (?, ?)',
      [1, JSON.stringify(currentSettings)]
    );
